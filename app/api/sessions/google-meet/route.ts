...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!sessionData.title || !sessionData.topic || !sessionData.start_time || !sessionData.end_time || !sessionData.google_meet_url) {
      return NextResponse.json({ error: 'Missing required fields: title, topic, start_time, end_time, and google_meet_url are required' }, { status: 400 });
    }
